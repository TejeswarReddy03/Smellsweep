...
#     regex_numeric_with_commas = re.compile(r'[,.].*[,.]') 
#     # re.compile(r'^\d+(?:[.,]\d+)?$')
...
#             # Check for separating smell in the numeric cells
#             separating_count = numeric_cells.astype(str).str.contains(regex_numeric_with_commas, na=False).sum()
#             separating_percentage = (separating_count / len(numeric_cells)) * 100
...
def identify_separating_smell(data):
    ...
    # Define regular expression for identifying cells with separators (`,` or `.`)
    regex_separators = re.compile(r'[,.]')
    ...
    # Loop through columns and rows to check for separating smell
    for column in data.columns:
        for index, value in data[column].items():
            # Check for separating smell in the cell
            if regex_separators.search(str(value)):
                metrics['Number of data points with separating smell'] += 1
                metrics['Percentage of separating smell per attribute'].setdefault(column, 0)
                metrics['Percentage of separating smell per attribute'][column] += 1

    # Calculate percentages
    total_data_points = data.shape[0] * data.shape[1]
    for column in metrics['Percentage of separating smell per attribute']:
        metrics['Percentage of separating smell per attribute'][column] /= total_data_points
        metrics['Percentage of separating smell per attribute'][column] *= 100
